...
%               Belgrade, 2019.
%--------------------------------------------------------------------------
function PlotThicknessNodeStrains(FE, node, cs, LAYERS, SUBLAYERS)
    ...
    for layer = 1:LAYERS
        ...
        for micro = 1:SUBLAYERS+1
            ...
            if strcmp(cs,'XYZ') == 1
                PlotX1(count,:) = FE.StrainsNodalXYZ(:,micro,layer,node)';
            else
                PlotX1(count,:) = FE.StrainsNodalXYZ(:,micro,layer,node)';
            end
...
%% PLOT
    ...
    figure
%     suptitle('Strains Distribution through the Thickness of the Plate - NODE')
    plot1 = subplot(2,3,1);
