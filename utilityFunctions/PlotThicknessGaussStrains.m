...
%               Belgrade, 2019.
%--------------------------------------------------------------------------
function PlotThicknessGaussStrains(FE, gp, cs, LAYERS, SUBLAYERS)
    ...
    for layer = 1:LAYERS
        ...
        for sub = 1:SUBLAYERS+1
            ...
            if strcmp(cs, 'XYZ') == 1
                PlotX1(count,:) = FE.StrainsXYZ(:,sub,layer,gp)';
            else
                PlotX1(count,:) = FE.Strains123(:,sub,layer,gp)';
            end
...
%% PLOT
    ...
    figure
%     suptitle('Strains Distribution through the Thickness of the Plate - GAUSS')
    plot1 = subplot(2,3,1);
    plot(PlotX1(:,1),PlotY1)
    xlabel('EpsilonXX'); ylabel('z / h');
    grid on
%     set(gca,'XLim',[minX, maxX])
    
    plot2 = subplot(2,3,2);
    plot(PlotX1(:,2),PlotY1)
    xlabel('EpsilonYY'); ylabel('z / h');
    grid on
% 	set(gca,'XLim',[minX, maxX])
    
    plot3 = subplot(2,3,3);
    plot(PlotX1(:,3),PlotY1)
    xlabel('EpsilonZZ'); ylabel('z / h');
    grid on
% 	set(gca,'XLim',[minX, maxX])
    ...
    xlabel('GammaYZ'); ylabel('z / h');
    grid on
% 	set(gca,'XLim',[minX, maxX])
    
    plot5 = subplot(2,3,5);
    plot(PlotX1(:,5),PlotY1)
    xlabel('GammaXZ'); ylabel('z / h');
    grid on
% 	set(gca,'XLim',[minX, maxX])
    
    plot6 = subplot(2,3,6);
    plot(PlotX1(:,6),PlotY1)
    xlabel('GammaXY'); ylabel('z / h');
    grid on
% 	set(gca,'XLim',[minX, maxX])
